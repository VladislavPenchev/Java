...
public class Pizza {
    ...
    private void setName(String name) {
        if(name.equals(null) || name.isEmpty() || name.length() > 15){
            throw new IllegalArgumentException(this.INVALID_PIZZA_NAME_EXCEPTION_MESSAGE);
        }
        this.name = name;
    }
    ...
    private void setToppings(int countOfToppings) {
        if(countOfToppings < 0 || countOfToppings > 10)
            throw new IllegalArgumentException(this.INVALID_COUNT_OF_TOPPINGS_EXCEPTION_MESSAGE);
